...
void parsecraylib_check_events(Parsec *parsec, void handle_event(ParsecGuest *, void *), void *ctx) {
	if (parsec != NULL) {
		for (ParsecHostEvent event; ParsecHostPollEvents(parsec, 0, &event);) {
			if (event.type == HOST_EVENT_GUEST_STATE_CHANGE) {
				handle_event(&event.guestStateChange.guest, ctx);
			}
		}
	}
}

void parsecraylib_check_input(Parsec *parsec, void handle_input(ParsecGuest *, ParsecMessage *, void *), void *ctx) {
	ParsecGuest guest;
	if (parsec != NULL) {
		for (ParsecMessage msg; ParsecHostPollInput(parsec, 0, &guest, &msg);) {
			handle_input(&guest, &msg, ctx);
		}
	}
}
#endif
