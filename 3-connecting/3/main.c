...
struct Player *game_get_player_from_guest(struct Game *game, ParsecGuest *guest) {
	if (guest == NULL) {
		return NULL;
	}
	for (int i = 0; i < NUM_PLAYERS; i++) {
...
void handle_parsec_guest_event(ParsecGuest *guest, void *ctx) {
	struct Game *game = (struct Game *) ctx;
	struct Player *player = game_get_player_from_guest(game, guest);
	if (guest->state == GUEST_CONNECTED) {
		game_add_player(game, guest);
...
void handle_parsec_input_event(ParsecGuest *guest, ParsecMessage *msg, void *ctx) {
	struct Game *game = (struct Game *) ctx;
	struct Player *player = game_get_player_from_guest(game, guest);
	if (player == NULL) {
		return;
	...
	if (msg->type == MESSAGE_KEYBOARD) {
		bool pressed = msg->keyboard.pressed;
		switch (msg->keyboard.code) {
			case PARSEC_KEY_UP:
				player->p_up = pressed;
				break;
			case PARSEC_KEY_DOWN:
				player->p_down = pressed;
				break;
			case PARSEC_KEY_LEFT:
				player->p_left = pressed;
				break;
			case PARSEC_KEY_RIGHT:
				player->p_right = pressed;
				break;
			default:
				break;
		}
...
void game_update(struct Game *game) {
	...
	parsecraylib_check_events(game->parsec, handle_parsec_guest_event, game);
	parsecraylib_check_input(game->parsec, handle_parsec_input_event, game);
}
